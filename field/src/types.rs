    ...
    + DeserializeOwned
{
    ...
    ///计算子群的生成元,如果子群的阶为t,又因为(g^t)^(n/t)=1,所以(g^(n/t))^t=1。所以子群的生成元为g^(n/t)
    ...
    fn two_adic_subgroup(n_log: usize) -> Vec<Self> {
        ...
        /*take 是 Rust 中迭代器的一种方法，用于从迭代器中获取前 n 个元素，
        并返回一个新的迭代器。这个方法不会消耗原始迭代器，而是返回一个新的迭代器，
        该迭代器只会生成前 n 个元素
         */
...
mod tests {
    ...
    fn test_subgroup(){
        type F = GoldilocksField;
        let subgroup = F::two_adic_subgroup(2);
        let inverses: Vec<F> = subgroup.iter().map(|&x| x.inverse()).collect();
        // 4阶subgroup: [1, 281474976710656, 18446744069414584320, 18446462594437873665]
        // 4阶inverses: [1, 18446462594437873665, 18446744069414584320, 281474976710656]
        assert_eq!(subgroup[1],inverses[3]);
        assert_eq!(subgroup[2],inverses[2]);
        assert_eq!(subgroup[3],inverses[1]);

        let subgroup = F::two_adic_subgroup(4);
        let inverses: Vec<F> = subgroup.iter().map(|&x| x.inverse()).collect();
        /*
        16阶subgroup: [1, 4096, 16777216, 68719476736, 281474976710656, 1152921504606846976, 1099511627520, 4503599626321920,
        18446744069414584320, 18446744069414580225, 18446744069397807105, 18446744000695107585, 1844646259443787665, 17293822564807737345, 18446742969902956801, 18442240469788262401]
        16阶inverses: [1, 18442240469788262401, 18446742969902956801, 17293822564807737345, 18446462594437873665, 18446744000695107585, 18446744069397807105, 18446744069414580225,
        18446744069414584320, 4503599626321920, 109951162720, 1152921504606846976, 281474976710656, 68719476736, 16777216, 4096]
        */
        assert_eq!(subgroup[1],inverses[15]);
        assert_eq!(subgroup[2],inverses[14]);
        assert_eq!(subgroup[3],inverses[13]);

        let subgroup = F::two_adic_subgroup(5);
        let inverses: Vec<F> = subgroup.iter().map(|&x| x.inverse()).collect();
        /*
        32阶subgroup:
        [1, 64, 4096, 262144, 16777216, 1073741824, 68719476736, 4398046511104,
        281474976710656, 18014398509481984, 1152921504606846976, 17179869180, 1099511627520, 70368744161280, 4503599626321920, 28823037608460288,
        18446744069414584320, 18446744069414584257, 18446744069414580225, 18446744069414322177, 18446744069397807105, 18446744068340842497, 18446744000695107585,
        18446739671368073217, 18446462594437873665, 18428729670905102337, 17293822564807737345, 18446744052234715141, 18446742969902956801, 18446673700670423041,
        18442240469788262401, 18158513693329981441]
        32阶inverses:
         [1, 18158513693329981441, 18442240469788262401, 18446673700670423041, 18446742969902956801, 18446744052234715141, 17293822564807737345,
        18428729670905102337, 18446462594437873665, 18446739671368073217, 1844674000695107585, 18446744068340842497, 18446744069397807105,
        18446744069414322177, 18446744069414580225, 18446744069414584257, 18446744069414584320, 288230376084602880, 4503599626321920,
        70368744161280, 1099511627520, 17179869180, 1152921504606846976, 18014398509481984, 281474976710656, 4398046511104, 68719476736,
        1073741824, 16777216, 262144, 4096, 64]
         */
        assert_eq!(subgroup[1],inverses[31]);
        assert_eq!(subgroup[2],inverses[30]);
        assert_eq!(subgroup[3],inverses[29]);
